/* ══════════════════════════════════════════════════════════════
   AI Benchmarking System — App (Complete SPA with 5-page flow)
   Flow: Home → Projects → Project Analytics → Versions → Run Detail
   ══════════════════════════════════════════════════════════════ */

const API = window.location.origin;

// Cyan/Teal palette for charts (matches styles.css scheme)
...
function navigateTo(view, p = {}) {
  ...
  switch (view) {
    ...
    // ── Page 3: Project Analytics Dashboard ──
    case 'project-dashboard':
      S.pid = p.projectId || S.pid; S.pname = p.projectName || S.pname;
      show('view-project-dashboard'); activate('nav-projects');
      setBc([
        { l: 'Projects', fn: () => navigateTo('projects') },
        { l: S.pname, fn: () => navigateTo('runs', { projectId: S.pid, projectName: S.pname }) },
        { l: 'Analytics Dashboard', active: true },
      ]);
      loadProjectDashboard(S.pid);
      break;

    // ── Page 4: Versions / Runs List ──
    case 'runs':
      S.pid = p.projectId || S.pid; S.pname = p.projectName || S.pname;
    ...
    case 'compare':
      S.pid = p.projectId || S.pid; S.pname = p.projectName || S.pname;
      show('view-compare'); activate('nav-projects');
      document.getElementById('compare-title').textContent = `Compare — ${S.pname}`;
      document.getElementById('compare-desc').textContent = 'Side-by-side comparison of all versions';
      setBc([
        { l: 'Projects', fn: () => navigateTo('projects') },
        { l: S.pname, fn: () => navigateTo('runs', { projectId: S.pid, projectName: S.pname }) },
        { l: 'Compare', active: true },
      ]);
      loadComparison(S.pid);
      break;
...
// Quick compare from sidebar — uses current project or first available
function quickCompare() {
  if (S.pid) navigateTo('compare', { projectId: S.pid, projectName: S.pname });
  else if (S.projects.length > 0) navigateTo('compare', { projectId: S.projects[0].id, projectName: S.projects[0].project_name });
...
function refreshCurrentView() {
  ...
  switch (S.view) {
    ...
    case 'project-dashboard':  loadProjectDashboard(S.pid); break;
    case 'runs':               loadRuns(S.pid); break;
    case 'run-detail':         loadRunDetail(S.pid, S.rid); break;
    case 'compare':            loadComparison(S.pid); break;
...
// ═══════════════════════════════════════════
// PAGE 3: PROJECT ANALYTICS DASHBOARD
// Shows: KPIs, Trend chart, Donut, Improvement cards, Bar chart, Param diff, Dataset
// ═══════════════════════════════════════════
async function loadProjectDashboard(pid) {
  document.getElementById('pd-proj-name').textContent = S.pname || 'Project';

  // Loading state for KPIs
  ['pd-kpi-best','pd-kpi-runs','pd-kpi-improvement','pd-kpi-latest'].forEach(id => {
    document.getElementById(id).textContent = '…';
  });

  // Loading state for chart containers
  ['pd-trend-chart','pd-bar-chart','pd-improvement-cards','pd-params-table','pd-dataset-overview'].forEach(id => {
  ...
  try {
    ...
    if (!runs.length) {
      ['pd-kpi-best','pd-kpi-runs','pd-kpi-improvement','pd-kpi-latest'].forEach(id => document.getElementById(id).textContent = '—');
      toast('No runs found for this project', 'inf');
      return;
    }

    // ── Determine primary metric (highest priority available) ──
    ...
    if (primaryKey) {
      ...
      document.getElementById('pd-kpi-best').textContent = fmtM(bestVal);
      document.getElementById('pd-kpi-improvement').textContent = improvement !== null
        ? (improvement >= 0 ? '+' : '') + improvement.toFixed(1) + '%' : '—';
    } else {
      document.getElementById('pd-kpi-best').textContent = '—';
      document.getElementById('pd-kpi-improvement').textContent = '—';
    }
    document.getElementById('pd-kpi-runs').textContent = runs.length;
    document.getElementById('pd-kpi-latest').textContent = runs[runs.length - 1]?.version_name || '—';

    // ── Render all visualizations ──
    renderPDTrendChart(runs, allMetricNames);
    renderPDImprovementCards(runs, allMetricNames, primaryKey);
    renderPDBarChart(runs, allMetricNames);
    renderPDParamsTable(runs);
    renderPDDatasetOverview(runs);
...
// ── Trend Line Chart (multi-metric, SVG) ──
function renderPDTrendChart(runs, allMetricNames) {
  const container = document.getElementById('pd-trend-chart');
  if (!container || !runs.length) return;

  const sub = document.getElementById('pd-trend-subtitle');
  ...
  let allVals = Object.values(series).flatMap(s => s.map(p => p.v).filter(v => v !== null));
  ...
  allMetricNames.forEach((mn, mi) => {
    ...
    const pts = series[mn].map((p, i) => p.v !== null ? { x: xPos(i), y: yPos(p.v), v: p.v, ver: p.ver } : null).filter(Boolean);
  ...
  container.innerHTML = svg + `<div class="chart-tooltip" id="pd-tt"></div>` + legend;

  const tt = document.getElementById('pd-tt');
...
// ── Grouped Bar Chart (all metrics × all versions) ──
function renderPDBarChart(runs, allMetricNames) {
  const container = document.getElementById('pd-bar-chart');
  if (!container || !runs.length) return;

  const metricsToShow = allMetricNames.slice(0, 5);
  if (!metricsToShow.length) { container.innerHTML = '<div class="empty-msg">No metrics available</div>'; return; }

  const W = 540, H = 260, pad = { t: 28, r: 16, b: 44, l: 44 };
  const cw = W - pad.l - pad.r, ch = H - pad.t - pad.b;

  const maxVal = 1; // Assuming primary metrics are 0.0 - 1.0 (accuracies, f1, precision, etc.)

  let svg = `<div class="pd-bar-inner"><div class="pd-bar-svg-wrap">`;
  svg += `<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet">`;

  // Grid lines & Y axis (0 - 100%)
  for (let i = 0; i <= 5; i++) {
    const y = pad.t + (ch / 5) * i;
    const v = maxVal - ((maxVal - 0) / 5) * i;
    svg += `<line x1="${pad.l}" y1="${y}" x2="${W - pad.r}" y2="${y}" class="chart-grid-line"/>`;
    svg += `<text x="${pad.l - 8}" y="${y + 4}" text-anchor="end" class="chart-axis-label">${(v * 100).toFixed(0)}%</text>`;
  }

  // X axis labels & Bars
  const numVersions = runs.length;
  const numMetrics = metricsToShow.length;
  const groupWidth = cw / numVersions;
  const barPadding = 8;
  const totalBarSpace = groupWidth - (barPadding * 2);
  const barWidth = totalBarSpace / numMetrics;

  runs.forEach((r, ri) => {
    const cx = pad.l + groupWidth * ri + groupWidth / 2;
    // Version name on X axis
    svg += `<text x="${cx}" y="${H - 6}" text-anchor="middle" class="chart-axis-label" style="fill:var(--tx2);font-weight:600">${esc(r.version_name)}</text>`;

    if (ri > 0) {
      const sepX = pad.l + groupWidth * ri;
      svg += `<line x1="${sepX}" y1="${pad.t}" x2="${sepX}" y2="${pad.t + ch}" stroke="rgba(255,255,255,.025)" stroke-width="1"/>`;
    }

    metricsToShow.forEach((mn, mi) => {
      const m = (r.metrics || []).find(x => x.metric_name === mn);
      let val = m ? parseFloat(m.metric_value) : 0;
      const displayVal = val <= 1 ? (val * 100).toFixed(2) + '%' : val.toFixed(4);
      val = Math.min(Math.max(val, 0), maxVal); // Clamp for UI
      
      const barH = (val / maxVal) * ch;
      const x = pad.l + groupWidth * ri + barPadding + mi * barWidth;
      const y = pad.t + ch - barH;
      const color = VERSION_COLORS[mi % VERSION_COLORS.length];
      
      const r_w = Math.max(barWidth - 2, 2);
      const rx = Math.min(3, barH);
      const d = barH > 0 
        ? `M${x},${pad.t + ch} L${x},${y + rx} Q${x},${y} ${x + rx},${y} L${x + r_w - rx},${y} Q${x + r_w},${y} ${x + r_w},${y + rx} L${x + r_w},${pad.t + ch} Z`
        : '';
      
      if (d) {
        svg += `<path d="${d}" fill="${color}" opacity="0.9" style="transition:all var(--fast);cursor:pointer;" onmouseover="this.setAttribute('opacity','1');this.setAttribute('fill','#fff')" onmouseout="this.setAttribute('opacity','0.9');this.setAttribute('fill','${color}')"><title>${capitalize(mn)}: ${displayVal}</title></path>`;
      }
    });
  });
  ...
  svg += `<div class="pd-bar-legend">`;
  metricsToShow.forEach((mn, mi) => {
    const color = VERSION_COLORS[mi % VERSION_COLORS.length];
    svg += `<div class="pd-bar-legend-item"><div style="width:10px;height:10px;border-radius:3px;background:${color};flex-shrink:0"></div>${capitalize(mn)}</div>`;
  });
  svg += `</div></div>`;
...
function renderPDParamsTable(runs) {
  const container = document.getElementById('pd-params-table');
  if (!container) return;
  ...
  runs.forEach((r, i) => { html += `<th class="version-header" style="color:${VERSION_COLORS[i % VERSION_COLORS.length]};border-color:${VERSION_COLORS[i % VERSION_COLORS.length]}">${esc(r.version_name)}</th>`; });
  html += '</tr></thead><tbody>';
  allParams.forEach(pn => {
...
function renderPDDatasetOverview(runs) {
  const container = document.getElementById('pd-dataset-overview');
  if (!container) return;
...
// COMPARISON MODULE
// ═══════════════════════════════════════════
async function loadComparison(pid) {
  try {
    const data = await api(`/projects/${pid}/compare`);
    const runs = (data.runs || []).sort((a, b) => a.version_name.localeCompare(b.version_name, undefined, { numeric: true }));
    if (!runs.length) { document.getElementById('compare-legend').innerHTML = '<div class="empty-msg">No runs to compare.</div>'; return; }
    renderLegend(runs); renderTopMetrics(runs); renderTrendChart(runs); renderBarChart(runs); renderParamDiff(runs); renderDatasetDiff(runs);
  } catch (err) { console.error(err); toast('Failed to load comparison', 'err'); }
}

function renderLegend(runs) {
  document.getElementById('compare-legend').innerHTML =
    runs.map((r, i) => `<div class="legend-item"><div class="legend-dot" style="background:${VERSION_COLORS[i % VERSION_COLORS.length]}"></div>${esc(r.version_name)}</div>`).join('');
}

function renderTopMetrics(runs) {
  const container = document.getElementById('cmp-metrics-row'); container.innerHTML = '';
  const allMetrics = [...new Set(runs.flatMap(r => (r.metrics || []).map(m => m.metric_name)))].slice(0, 4);
  let html = '';
  allMetrics.forEach(mn => {
    const values = runs.map(r => { const m = (r.metrics || []).find(x => x.metric_name === mn); return m ? parseFloat(m.metric_value) : null; });
    ...
    const pts = values.map((v, i) => v !== null ? { x: (i / (runs.length - 1 || 1)) * Wc, y: Hc - ((v - minM) / (maxM - minM || 1)) * Hc } : null).filter(Boolean);
    let miSvg = `<svg viewBox="-5 -5 ${Wc + 10} ${Hc + 10}" preserveAspectRatio="none">`;
    if (pts.length > 1) miSvg += `<path d="${pts.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x},${p.y}`).join(' ')}" class="cmc-line" stroke="${VERSION_COLORS[0]}" fill="none"/>`;
    ...
    html += `<div class="cmp-metric-card"><div class="cmc-head"><div class="cmc-title">${capitalize(mn)}</div><div class="cmc-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg></div></div><div class="cmc-val">${dv}</div>${diffHtml}<div class="cmc-chart">${miSvg}</div></div>`;
  });
  container.innerHTML = html;
}

function renderTrendChart(runs) {
  const container = document.getElementById('chart-trend'); if (!container) return;
  const allMetrics = [...new Set(runs.flatMap(r => (r.metrics || []).map(m => m.metric_name)))];
  const series = {};
  allMetrics.forEach(mn => { series[mn] = runs.map(r => { const m = (r.metrics || []).find(x => x.metric_name === mn); return { v: m ? parseFloat(m.metric_value) : null, ver: r.version_name }; }); });
  const W = 740, H = 270, pad = { t: 28, r: 24, b: 44, l: 54 };
  const cw = W - pad.l - pad.r, ch = H - pad.t - pad.b;
  let allVals = Object.values(series).flatMap(s => s.map(p => p.v).filter(v => v !== null));
  if (!allVals.length) { container.innerHTML = '<div class="empty-msg">No metric data</div>'; return; }
  let minV = Math.min(...allVals), maxV = Math.max(...allVals);
  const rng = maxV - minV || 1; minV = Math.max(0, minV - rng * .1); maxV += rng * .1;
  const xPos = i => pad.l + (runs.length > 1 ? (i / (runs.length - 1)) * cw : cw / 2);
  const yPos = v => pad.t + ch - ((v - minV) / (maxV - minV)) * ch;
  let svg = `<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet" style="width:100%;height:100%">`;
  for (let i = 0; i <= 5; i++) { const y = pad.t + (ch / 5) * i; const v = maxV - ((maxV - minV) / 5) * i; svg += `<line x1="${pad.l}" y1="${y}" x2="${W - pad.r}" y2="${y}" class="chart-grid-line"/><text x="${pad.l - 8}" y="${y + 4}" text-anchor="end" class="chart-axis-label">${v.toFixed(3)}</text>`; }
  runs.forEach((r, i) => { const x = xPos(i); svg += `<line x1="${x}" y1="${pad.t}" x2="${x}" y2="${pad.t + ch}" stroke="rgba(255,255,255,.025)" stroke-width="1"/><text x="${x}" y="${H - 6}" text-anchor="middle" class="chart-axis-label" style="fill:var(--tx2);font-weight:600">${esc(r.version_name)}</text>`; });
  allMetrics.forEach((mn, mi) => { const color = VERSION_COLORS[mi % VERSION_COLORS.length]; const pts = series[mn].map((p, i) => p.v !== null ? { x: xPos(i), y: yPos(p.v), v: p.v, ver: p.ver } : null).filter(Boolean); if (!pts.length) return; const gid = `cg${mi}`; svg += `<defs><linearGradient id="${gid}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${color}" stop-opacity="0.25"/><stop offset="100%" stop-color="${color}" stop-opacity="0"/></linearGradient></defs>`; if (pts.length > 1) { const ap = `M${pts[0].x},${pad.t + ch} ` + pts.map(p => `L${p.x},${p.y}`).join(' ') + ` L${pts[pts.length - 1].x},${pad.t + ch} Z`; const lp = pts.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x},${p.y}`).join(' '); svg += `<path d="${ap}" fill="url(#${gid})" class="chart-area"/><path d="${lp}" fill="none" stroke="${color}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" opacity="0.9"/>`; } pts.forEach(p => { svg += `<circle cx="${p.x}" cy="${p.y}" r="4.5" fill="${color}" stroke="var(--bg-card)" stroke-width="2" class="chart-dot" data-metric="${esc(mn)}" data-val="${p.v}" data-ver="${esc(p.ver)}"/>`; }); });
  svg += '</svg>';
  const legend = `<div style="display:flex;flex-wrap:wrap;gap:12px;padding:10px 20px 14px;border-top:1px solid var(--bdr)">${allMetrics.map((mn, i) => `<div style="display:flex;align-items:center;gap:5px;font-size:.72rem;color:var(--tx2)"><div style="width:13px;height:3px;border-radius:2px;background:${VERSION_COLORS[i % VERSION_COLORS.length]}"></div>${capitalize(mn)}</div>`).join('')}</div>`;
  container.innerHTML = svg + `<div class="chart-tooltip" id="cmp-tt"></div>` + legend;
  const tt = document.getElementById('cmp-tt');
  container.querySelectorAll('.chart-dot').forEach(dot => { dot.addEventListener('mouseenter', () => { const sr = container.querySelector('svg').getBoundingClientRect(); const cx = parseFloat(dot.getAttribute('cx')) * (sr.width / W); const cy = parseFloat(dot.getAttribute('cy')) * (sr.height / H); tt.innerHTML = `<div class="tt-metric">${dot.dataset.metric}</div><div class="tt-val">${dot.dataset.ver}: ${dot.dataset.val}</div>`; tt.classList.add('show'); tt.style.left = (cx + 12) + 'px'; tt.style.top = (cy - 18) + 'px'; }); dot.addEventListener('mouseleave', () => tt.classList.remove('show')); });
}

function renderBarChart(runs) {
  const container = document.getElementById('chart-bars'); if (!container) return;
  const allMetrics = [...new Set(runs.flatMap(r => (r.metrics || []).map(m => m.metric_name)))].slice(0, 5);
  let allVals = []; runs.forEach(r => allMetrics.forEach(mn => { const m = (r.metrics || []).find(x => x.metric_name === mn); if (m) allVals.push(Math.abs(parseFloat(m.metric_value))); }));
  const maxVal = Math.max(...allVals, 0.01);
  let html = '';
  runs.forEach((r, ri) => { html += '<div class="bar-group"><div class="bar-group-bars">'; allMetrics.forEach((mn, mi) => { const m = (r.metrics || []).find(x => x.metric_name === mn); const v = m ? parseFloat(m.metric_value) : 0; const pct = Math.min((Math.abs(v) / maxVal) * 100, 100); const color = VERSION_COLORS[mi % VERSION_COLORS.length]; html += `<div class="bar-single" style="height:${Math.max(pct, 2)}%;background:${color};animation-delay:${ri * 80 + mi * 40}ms"><div class="bar-val">${v <= 1 ? (v * 100).toFixed(1) + '%' : v.toFixed(3)}</div></div>`; }); html += `</div><div class="bar-group-label">${esc(r.version_name)}</div></div>`; });
  container.innerHTML = html;
}

function renderParamDiff(runs) {
  const container = document.getElementById('param-diff');
  const allParams = [...new Set(runs.flatMap(r => (r.parameters || []).map(p => p.parameter_name)))];
  if (!allParams.length) { container.innerHTML = '<div class="empty-msg">No parameters to compare.</div>'; return; }
  const lookup = {}; allParams.forEach(pn => { lookup[pn] = {}; runs.forEach(r => { const p = (r.parameters || []).find(x => x.parameter_name === pn); lookup[pn][r.version_name] = p ? String(p.parameter_value) : '—'; }); });
  let html = `<table class="diff-table"><thead><tr><th>Parameter</th>`;
  runs.forEach((r, i) => { html += `<th class="version-header" style="color:${VERSION_COLORS[i % VERSION_COLORS.length]};border-color:${VERSION_COLORS[i % VERSION_COLORS.length]}">${esc(r.version_name)}</th>`; });
  html += '</tr></thead><tbody>';
  allParams.forEach(pn => { const vals = runs.map(r => lookup[pn][r.version_name]); html += `<tr><td class="param-name">${esc(pn)}</td>`; runs.forEach((r, i) => { const changed = i > 0 && vals[i] !== vals[i - 1]; html += `<td class="version-col${changed ? ' changed' : ''}">${esc(vals[i])}</td>`; }); html += '</tr>'; });
  html += '</tbody></table>'; container.innerHTML = html;
}

function renderDatasetDiff(runs) {
  const container = document.getElementById('dataset-diff');
  const fields = ['dataset_name', 'dataset_version', 'train_images', 'val_images', 'classes'];
  const labels = ['Dataset', 'Version', 'Train Images', 'Val Images', 'Classes'];
  let html = `<table class="cmp-info-table"><thead><tr><th>Field</th>`;
  runs.forEach((r, i) => { html += `<th class="version-header" style="color:${VERSION_COLORS[i % VERSION_COLORS.length]};border-color:${VERSION_COLORS[i % VERSION_COLORS.length]}">${esc(r.version_name)}</th>`; });
  html += '</tr></thead><tbody>';
  fields.forEach((f, fi) => { html += `<tr><td class="field-name">${labels[fi]}</td>`; runs.forEach(r => { const di = (r.dataset_info || [])[0]; let v = di ? di[f] : '—'; if (v != null && typeof v === 'number') v = v.toLocaleString(); html += `<td class="version-col">${esc(String(v ?? '—'))}</td>`; }); html += '</tr>'; });
  html += '</tbody></table>'; container.innerHTML = html;
}
